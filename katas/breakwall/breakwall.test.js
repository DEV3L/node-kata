...
const breakwall = require('./breakwall');
const { expect } = require('chai');

// test suite
describe('Breakwall', function() {
  // individual test
  it('Parse Policy no result', function() {
    // arrange
    const expected = { minCount: 0, maxCount: 0, value: "", password: "" }
    
    let policy = ""
    // act
    const result = breakwall.parsePolicy(policy)

    // assert
    expect(expected).to.eqls(result)
  });

  it('Parse Policy result', function() {
    // arrange
    
    const expected = { minCount: 1, maxCount: 3, value: "a", password: "abcde" }
    
    // act
    let policy = "1-3 a: abcde"
    const result = breakwall.parsePolicy(policy)
  ...
  it('Validate Policy no result', function() {
    // arrange
    const expected = false
    
    // act
    let policy = { minCount: 1, maxCount: 3, value: "a", password: "bcde" }
    const result = breakwall.validatePolicy(policy)

    // assert
    expect(expected).to.eqls(result)
  });

  it('Validate Policy result', function() {
    // arrange
    const expected = true
    
    // act
    let policy = { minCount: 1, maxCount: 3, value: "a", password: "abcde" }
    const result = breakwall.validatePolicy(policy)
  ...
  it('RUN', function() {
    ...
    // act
    for (let policyStringIndex in data)
    {
      let policyString = data[policyStringIndex]
      let policy = breakwall.parsePolicy(policyString)
      let isValid = breakwall.validatePolicy(policy)
